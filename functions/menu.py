...
def searchwindowset():
    ...
    BuyFrame = Tk()
    BuyFrame.configure(background=config.bgcolor)
    ...
    for d in result:
        if d['listing']['price'] != None:
          ...
          if 'corrupted' in d['item']:
              ...
              wr[r] = tk.Label(BuyFrame, text="price {} {} Corrupt - {}".format(amount, currency, nick),fg=config.textcolor, bg=config.bgcolor).grid(row=r)
              br[r] = tk.Button(BuyFrame, text ="Buy", bg=config.bgcolor, fg=config.fgcolor, command=lambda whisper=whisper: buyitem(whisper)).grid(row=r, column=1)

          else:
              wr[r] = tk.Label(BuyFrame, text="price {} {} - {}".format(amount, currency, nick), fg=config.textcolor, bg=config.bgcolor).grid(row=r)
              br[r] = tk.Button(BuyFrame, text ="Buy", bg=config.bgcolor, fg=config.fgcolor, command=lambda whisper=whisper: buyitem(whisper)).grid(row=r, column=1)

          r = r + 1
    btn1 = tk.Button(BuyFrame, text = "Show on web", bg=config.bgcolor, fg=config.fgcolor, command=lambda: webbrowser.open("https://www.pathofexile.com/trade/search/Metamorph/"+ query)).grid(row=r, column=0)
...
def createmainmenu():
    ...
    btn1 = tk.Button(menuwindow, text = "Search", bg=config.bgcolor, fg=config.fgcolor, command=searchwindowset).grid(row=1, column=3)
    ...
    bta1 = tk.Button(menuwindow, text = "reset", bg=config.bgcolor, fg=config.fgcolor, command=lambda: resetcount("redeemer")).grid(row=2, column=3)
    ...
    bta2 = tk.Button(menuwindow, text = "reset", bg=config.bgcolor, fg=config.fgcolor, command=lambda: resetcount("crusader")).grid(row=3, column=3)
    ...
    bta3 = tk.Button(menuwindow, text = "reset", bg=config.bgcolor, fg=config.fgcolor, command=lambda: resetcount("warlord")).grid(row=4, column=3)
    ...
    bta4 = tk.Button(menuwindow, text = "reset", bg=config.bgcolor, fg=config.fgcolor, command=lambda: resetcount("hunter")).grid(row=5, column=3)


    btnop = tk.Button(menuwindow, text = "Options", bg=config.bgcolor, fg=config.fgcolor, command=startopt).grid(row=6, column=1)
...
def creatoptions():
    ...
    w = tk.Label(optionwindow, text="Button Foreground Color", fg=config.textcolor, bg=config.bgcolor).grid(row=5, column=1)
    g4 = tk.Entry(optionwindow, width=30, fg=config.fgcolor, bg=config.bgcolor)
    g4.insert(0, config.fgcolor)
    g4.grid(row=5, column=2)
    btn5 = tk.Button(optionwindow, text = "Set", bg=config.bgcolor, fg=config.fgcolor, command=lambda: stcolor("fgcolor", g4))
    btn5.grid(row=5, column=3)

    w = tk.Label(optionwindow, text="Button Background Color", fg=config.textcolor, bg=config.bgcolor).grid(row=6, column=1)
    g5 = tk.Entry(optionwindow, width=30, fg=config.fgcolor, bg=config.bgcolor)
    g5.insert(0, config.bgcolor)
    g5.grid(row=6, column=2)
    btn5 = tk.Button(optionwindow, text = "Set", bg=config.bgcolor, fg=config.fgcolor, command=lambda: stcolor("bgcolor", g5))
    ...
    btn6 = tk.Button(optionwindow, text = "Set", bg=config.bgcolor, fg=config.fgcolor, command=lambda: stcolor("textcolor", g6))
